...
transform Shake(duration, distance, repeats):
    linear duration/4 xoffset distance
    linear duration/2 xoffset -distance*2
    linear duration/4 xoffset distance
    xpos 0.4878
    repeat repeats
...
screen say(who, what, window_transform = NullTransform):
    if(animate):
        on "hide" action With(Dissolve(0.3))
        on "show" action With(Dissolve(0.3))
    if(not mini):
        style_prefix "say"
...
style say_label:
    ...
    yalign 0.5
    outlines [(3, "#0089cd", 0, 0), (1, "#0089cd", 1, 1)]
...
style miniwindow is default
style minisay_label is default
style minisay_dialogue is default
style minisay_thought is minisay_dialogue

style namebox is default
style namebox_label is minisay_label
...
style miniwindow:
    ...
    yalign gui.textbox_yalign
    ysize 77
    background Image("gui/mbox.png", xalign=0.5, yalign=1.0)

style mnamebox:
    xpos gui.name_xpos
    xanchor gui.name_xalign
    xsize gui.namebox_width
    ypos gui.name_ypos - 36
    ysize gui.namebox_height
...
## Floating window
# to  show: show screen flwindow([location of the pic])
# e.g show screen flwindow("mod_assets/monika.png")
# to hide, hide screen flwindow
# flx defines the relative position in x
#the pic must have a size of less than 300x300
#for sprites, show screen flwindow("monika 1a",True)
define flx = 0.9
transform showfl:
    yalign 0.4
    alpha 0.0
    linear 0.2 yalign 0.3 alpha 1.0


screen flwindow(pic, issprite=False):
    on "show" action With(Dissolve(0.3))
    on "hide" action With(Dissolve(0.2))
    frame at showfl:
        style "flframe"
        if(issprite):
            add pic xalign 0.5 yalign 0.4 zoom 0.3125
        else: 
            add pic xalign 0.5 yalign 0.5
        

style flframe:
    background "gui/fl.png"
    xsize 300
    ysize 300
    yalign 0.3
    xalign flx

## NVL Mode for DDLC (Monologue thing)

transform nvl_tr:
    alpha 0.0
    linear 1.0 alpha 1.0

define nvl_enable = True
    ...
    vbox:
        ...
        if not persistent.autoload or not main_menu:
            ...
            if renpy.variant("pc"):
                ...
                textbutton _("Help") action Show(screen="dialog",  message="There's no help for you.", ok_action=Hide("dialog"))
...
#init python:
    #def FileActionMod(name, page=None, **kwargs):
            ...
            #return Show(screen="dialog", message="You can't escape.", ok_action=Hide("dialog"))
        #else:
